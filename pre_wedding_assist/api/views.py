...
from .serializers import WeddingSerializer, TableSerializer, GuestSerializer, StageSerializer, CreateWeddingSerializer, CreateTableSerializer, CreateGuestSerializer, CreateStageSerializer
...
class CreateStageView(APIView):
    serializer_class = CreateStageSerializer
...
class EditStageView(APIView):
    ...
    def post(self, request, format=None, id=None):
        ...
        if id != None:
            ...
            serializer = self.serializer_class(stage, data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            return Response({'Stage Not Found': 'Invalid Stage Id'},status=status.HTTP_404_NOT_FOUND)
        return Response({'Bad Request': 'Id paramater not found in request'},status=status.HTTP_400_BAD_REQUEST)
